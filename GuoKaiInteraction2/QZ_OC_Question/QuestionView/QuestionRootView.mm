...
//    <--
    ...
    [upButton setImage:[UIImage imageNamed:@"a1.png"] forState:UIControlStateNormal];
...
//    ==
    ...
    answerButton.frame = CGRectMake(frame.size.width/2-QUESTION_ANSWERBUTTON_WIDTH/2, frame.size.height-QUESTION_UPANDNEXT_HEIGHT, QUESTION_ANSWERBUTTON_WIDTH, QUESTION_UPANDNEXT_HEIGHT);
...
//    -->
    ...
    [nextButton setImage:[UIImage imageNamed:@"a2.png"] forState:UIControlStateNormal];
...
- (void)upPressButton:(UIButton *)button
{
    ...
    //  切换到指定的页面
    [scrollView setContentOffset:CGPointMake(self.frame.size.width * page, 0)];
...
- (void)nextPressButton:(UIButton *)button
{
    ...
    //  获取当前pagecontroll的值
    int page = curPageView + 1;
    if (page > (scrollView.contentSize.width/scrollView.frame.size.width-1))
    {
        page = scrollView.contentSize.width/scrollView.frame.size.width - 1;
...
{
    QuestionChoice *qChoice = [[QuestionChoice alloc]initWithFrame:CGRectMake(15 +frame.size.width *i, 0, frame.size.width-30, frame.size.height-QUESTION_DISTANT*2-titHeight-QUESTION_UPANDNEXT_HEIGHT)];
...
- (void)creatQuestionSort:(CGRect)frame withIndex:(NSInteger)i withQuestionData:(PageQuestionSort *)pQuestionSort
{
        ...
        QuestionSort *qSort = [[QuestionSort alloc]initWithFrame:CGRectMake(15 +frame.size.width *i, 0, frame.size.width-30, frame.size.height-QUESTION_DISTANT*2-titHeight-QUESTION_UPANDNEXT_HEIGHT)];
...
{
        QuestionDragPoint *qChoice = [[QuestionDragPoint alloc]initWithFrame:CGRectMake(15 +frame.size.width *i, 0, frame.size.width-30, frame.size.height-QUESTION_DISTANT*2-titHeight-QUESTION_UPANDNEXT_HEIGHT)];
